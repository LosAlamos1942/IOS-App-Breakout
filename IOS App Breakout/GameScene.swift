...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func kickBall() {
        ...
        ball.physicsBody?.applyImpulse(CGVector(dx: Int.random(in: -5...5),
                                                dy: 5))
    }
    ...
    func makePaddle() {
        ...
        paddle = SKSpriteNode(color: .white, size: CGSize(width:
    ...
    func makeBricks() {
        ...
        let xOffset = (Int(frame.width) - (count * 55)) / 2 + Int(frame.minX)
            + 30
        let colors: [UIColor] = [.red, .gray, .yellow]
        for r in 0..<3 {
            let y = Int(frame.maxY) - 21 - (r * 25)
            for i in 0..<count {
                let x = i * 55 + xOffset
    ...
    func makeLoseZone() {
        loseZone = SKSpriteNode(color: .red, size: CGSize(width: frame.width,
                                                          height: 50))
    ...
    func makeLabels() {
        ...
        livesLabel.fontName = "Times New Roman"
    ...
    override func update(_ currentTime: TimeInterval) {
        if abs(ball.physicsBody!.velocity.dx) < 100 {
            ...
            ball.physicsBody?.applyImpulse(CGVector(dx: Int.random(in:
                                                                    -3...3), dy: 0))
        }
        if abs(ball.physicsBody!.velocity.dy) < 100 {
            // ball has stalled in y direct, so kick it randomly vertically
            ball.physicsBody?.applyImpulse(CGVector(dx: 0, dy: Int.random(in:
                                                                            -3...3)))
        }
    }
}
